	...
	function admin_edit($id = null) {
		if(isset($this->data) && !empty($this->data)) {
			if($this->Type->save($this->data)) {
				foreach($this->request->data['StatsType'] as $index => $StatsType) {
					if($StatsType['check'] != 1) {
						unset($this->request->data['StatsType'][$index]);
					}
					else {
						$this->request->data['StatsType'][$index]['type_id'] = $this->request->data['Type']['id'];
					}
				}
				$this->Type->StatsType->deleteAll(array('type_id' => $id));
				$this->Type->StatsType->saveAll($this->request->data['StatsType']);
			}
			$this->redirect('/admin/types');
		}
		$this->javascripts[] = '/js/tiny_mce/jquery.tinymce.js';
