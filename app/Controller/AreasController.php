	...
	function admin_add($map_id = null, $direction = 'right') {
		...
		switch($direction) {
			...
			for($i = 0; $i < $mapInfo['y']; $i++) {
				$this->request->data['Area'][$index]['access'] = 1;
				$this->request->data['Area'][$index]['x'] = $mapInfo['x'] + 1;
				$this->request->data['Area'][$index]['y'] = $i + 1;
				$this->request->data['Area'][$index]['tile_id'] = 1;
				$this->request->data['Area'][$index]['map_id'] = $map['Map']['id'];
				$index++;
			}
			$this->Area->saveAll($this->request->data['Area']);
			break;

			case 'bottom':
			for($i = 0; $i < $mapInfo['x']; $i++) {
				$this->request->data['Area'][$index]['access'] = 1;
				$this->request->data['Area'][$index]['y'] = $mapInfo['y'] + 1;
				$this->request->data['Area'][$index]['x'] = $i + 1;
				$this->request->data['Area'][$index]['tile_id'] = 1;
				$this->request->data['Area'][$index]['map_id'] = $map['Map']['id'];
				$index++;
			}
			...
			for($i = 0; $i < $mapInfo['x']; $i++) {
				$this->request->data['Area'][$index]['access'] = 1;
				$this->request->data['Area'][$index]['y'] = 1;
				$this->request->data['Area'][$index]['x'] = $i + 1;
				$this->request->data['Area'][$index]['tile_id'] = 1;
				$this->request->data['Area'][$index]['map_id'] = $map['Map']['id'];
				$index++;
			...
			for($i = 0; $i < $mapInfo['y']; $i++) {
				$this->request->data['Area'][$index]['access'] = 1;
				$this->request->data['Area'][$index]['x'] = 1;
				$this->request->data['Area'][$index]['y'] = $i + 1;
				$this->request->data['Area'][$index]['tile_id'] = 1;
				$this->request->data['Area'][$index]['map_id'] = $map['Map']['id'];
				$index++;
