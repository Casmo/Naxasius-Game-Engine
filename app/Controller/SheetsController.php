...
class SheetsController extends AppController {
	...
	function admin_add_to_area($area_id = null) {
		if(isset($this->data) && !empty($this->data)) {
			$this->render(false);
			if(isset($this->request->data['Sheet']['id'])) {
				$someSheet = $this->Sheet->find('first', array('conditions' => array('Sheet.id' => $this->request->data['Sheet']['id'])));
				if(empty($someSheet)) {
					return false;
				}
				...
				$this->loadModel('Area');
				$this->Area->contain();
				$topLeftArea = $this->Area->find('first', array('conditions' => array('Area.id' => $this->request->data['Sheet']['area_id'])));
							...
							if(!empty($currentArea)) {
								foreach($obstaclesPosition[$x][$y] as $obstacle) {
									$saveObstacles[] = array(
										...
										'x' => $this->request->data['Sheet']['x'],
										'y' => $this->request->data['Sheet']['y'],
										'z' => $this->request->data['Sheet']['z']
									);
								}
							}
						}
	...
	function admin_save() {
		if(isset($this->data) && !empty($this->data)) {
			if($this->Sheet->save($this->data)) {
				if(isset($this->request->data['ObstaclesSheet'])) {
					foreach($this->request->data['ObstaclesSheet'] as $index => $obstacle) {
						if(!isset($obstacle['obstacle_id'])) {
							unset($this->request->data['ObstaclesSheet'][$index]);
						}
						else {
							$this->request->data['ObstaclesSheet'][$index]['sheet_id'] = $this->Sheet->id;
						}
					}
					$this->Sheet->ObstaclesSheet->deleteAll(array('ObstaclesSheet.sheet_id' => $this->Sheet->id));
					$this->Sheet->ObstaclesSheet->saveAll($this->request->data['ObstaclesSheet']);
				}
			}
		}
		$this->redirect('/admin/sheets');
	...
	function admin_delete($id = null) {
		...
		elseif(isset($this->request->data['Sheet']['id'])) {
			$this->deleteAll($this->request->data['Sheet']['id']);
		}
		$this->redirect('/admin/sheets');
	}
