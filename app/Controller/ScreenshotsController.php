	...
	function admin_save() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->request->data['Text']['id'])) {
				$screenshot = $this->Screenshot->find('first', array('conditions' => array('Screenshot.id' => $this->request->data['Screenshot']['id'])));
			}
			// resize
			App::import('Vendor', 'Image', array('file' => 'image.php'));
			$Image = new Image();
			if(empty($this->request->data['Screenshot']['image']['name'])) {
				unset($this->request->data['Screenshot']['image']);
			}
			else {
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->request->data['Screenshot']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->request->data['Screenshot']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'website'. DS .'screenshots'. DS . 'original'. DS . $imageName);
				...
				$this->request->data['Screenshot']['image'] = $imageName;
