	...
	function admin_save() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['News']['id'])) {
				$news = $this->News->find('first', array('conditions' => array('News.id' => $this->data['News']['id'])));
			}
			else {
				$this->data['News']['user_id'] = $this->userInfo['id'];
			}
			// resize
			...
			if(empty($this->data['News']['image']['name'])) {
				...
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['News']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['News']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'website'. DS .'news'. DS . $imageName);
				$Image->resize(WWW_ROOT . 'img'. DS .'website'. DS .'news'. DS . $imageName, WWW_ROOT . 'img'. DS .'website'. DS .'news'. DS . $imageName, 64, 64);
				$this->data['News']['image'] = $imageName;
