...
class RepliesController extends AppController {
	...
	function add() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Reply']['news_id'])) {
				$someNews = $this->Reply->News->find('first', array('conditions' => array('News.id' => $this->data['Reply']['news_id'])));
				if(empty($someNews)) {
					$this->redirect('/news');
				}
				else {
					$redirect = 'news';
					$this->data['Reply']['type'] = 'news';
					$this->data['Reply']['target_id'] = $this->data['Reply']['news_id'];
					unset($this->data['Reply']['news_id']);
				}
			}
			if(isset($this->data['Reply']['topic_id'])) {
				$someTopic = $this->Reply->Forum->find('first', array('conditions' => array('Topic.id' => $this->data['Reply']['topic_id'])));
				if(empty($someTopic)) {
					$this->redirect('/forum');
				}
				else {
