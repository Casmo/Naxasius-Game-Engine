...
class QuestsController extends AppController {
	...
	function admin_edit($id = null) {
		if(isset($this->data) && !empty($this->data)) {
			if($this->Quest->save($this->data)) {
				foreach($this->data['ItemsQuest'] as $index => $ItemsQuest) {
					if($ItemsQuest['check'] != 1) {
						unset($this->data['ItemsQuest'][$index]);
					}
					else {
						$this->data['ItemsQuest'][$index]['quest_id'] = $id;
					}
				}
				$this->Quest->ItemsQuest->deleteAll(array('quest_id' => $id));
				$this->Quest->ItemsQuest->saveAll($this->data['ItemsQuest']);

				foreach($this->data['QuestsStat'] as $index => $QuestsStat) {
					if($QuestsStat['check'] != 1) {
						unset($this->data['QuestsStat'][$index]);
					}
					else {
						$this->data['QuestsStat'][$index]['quest_id'] = $id;
					}
				}
				$this->Quest->QuestsStat->deleteAll(array('quest_id' => $id));
				$this->Quest->QuestsStat->saveAll($this->data['QuestsStat']);

				foreach($this->data['MobsQuest'] as $index => $MobsQuest) {
					if($MobsQuest['check'] != 1) {
						unset($this->data['MobsQuest'][$index]);
					}
					else {
						$this->data['MobsQuest'][$index]['quest_id'] = $id;
					}
				}
				$this->Quest->MobsQuest->deleteAll(array('quest_id' => $id));
				$this->Quest->MobsQuest->saveAll($this->data['MobsQuest']);
				$this->redirect('/admin/quests');
			}
		}
		else {
