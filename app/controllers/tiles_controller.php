...
class TilesController extends AppController {
	...
	function admin_search() {
		...
		if(isset($this->data) && !empty($this->data)) {
			$tiles = $this->Tile->find('all', array('conditions' => array('Group.id' => $this->data['Group']['id'])));
		}
		$this->set('tiles', $tiles);
	...
	function admin_save() {
		...
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Tile']['id']) && !empty($this->data['Tile']['id'])) {
				$tile = $this->Tile->find('first', array('conditions' => array('Tile.id' => $this->data['Tile']['id'])));
			}
			if(empty($this->data['Tile']['image']['name'])) {
				unset($this->data['Tile']['image']);
			}
			else {
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['Tile']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['Tile']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'game'. DS .'tiles'. DS . $imageName);
				$this->data['Tile']['image'] = $imageName;
				if(isset($tile['Tile']['image'])) {
					@unlink(WWW_ROOT . 'img'. DS .'game'. DS .'tiles'. DS . $tile['Tile']['image']);
				}
				$this->data['Tile']['sha1'] = sha1_file(WWW_ROOT . 'img'. DS .'game'. DS .'tiles'. DS . $imageName);
			}
	...
	function admin_delete($id = null) {
		...
		elseif(isset($this->data['Tile']['id'])) {
			$this->deleteAll($this->data['Tile']['id']);
		}
		$this->redirect('/admin/tiles');
	}
	...
	function admin_import() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Tile']['save']) && $this->data['Tile']['save'] == '1') {
				$saveData = array();
				foreach($this->data['Tile']['images'] as $image) {
					if(rename(WWW_ROOT . 'img' . DS . 'admin' . DS . 'tmp' . DS . $image, WWW_ROOT . 'img' . DS . 'game' . DS . 'tiles' . DS . $image)) {
						...
						$saveData[] = array('name' => '', 'image' => $image, 'group_id' => $this->data['Tile']['group_id'], 'sha1' => $sha1);
					}
				}
				...
				$sizes = getimagesize($this->data['Tile']['image']['tmp_name']);
				$src_im = imagecreatefrompng($this->data['Tile']['image']['tmp_name']);
