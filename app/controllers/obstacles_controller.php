...
class ObstaclesController extends AppController {
	...
	function admin_index() {
		$filters = array();
		if(isset($this->data['Group']['id']) && $this->data['Group']['id'] != 0) {
			$filters['Group.id'] = $this->data['Group']['id'];
		}
		elseif(isset($this->passedArgs['Group.id']) && $this->passedArgs['Group.id'] != 0) {
			$filters['Group.id'] = $this->passedArgs['Group.id'];
		}
		elseif(isset($this->data['Group']['id']) && $this->data['Group']['id'] == 0) {
	...
	function admin_search() {
		...
		if(isset($this->data) && !empty($this->data)) {
			$obstacles = $this->Obstacle->find('all', array('conditions' => array('Group.id' => $this->data['Group']['id'])));
		}
		$groups = $this->Obstacle->Group->find('list', array('conditions' => array('type' => 'obstacle')));
		$this->set('groups', $groups);
		$this->set('obstacles', $obstacles);
	...
	function admin_save($id = null) {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Obstacle']['id']) && !empty($this->data['Obstacle']['id'])) {
				$obstacle = $this->Obstacle->find('first', array('conditions' => array('Obstacle.id' => $this->data['Obstacle']['id'])));
			}
			if(empty($this->data['Obstacle']['image']['name'])) {
				unset($this->data['Obstacle']['image']);
			}
			else {
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['Obstacle']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['Obstacle']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'game'. DS .'obstacles'. DS . $imageName);
				$this->data['Obstacle']['image'] = $imageName;
				if(isset($obstacle['Obstacle']['image'])) {
					@unlink(WWW_ROOT . 'img'. DS .'game'. DS .'obstacles'. DS . $obstacle['Obstacle']['image']);
				}
				$this->data['Obstacle']['sha1'] = sha1_file(WWW_ROOT . 'img'. DS .'game'. DS .'obstacles'. DS . $imageName);
			}
	...
	function admin_delete($id = null) {
		...
		elseif(isset($this->data['Obstacle']['id'])) {
			$this->deleteAll($this->data['Obstacle']['id']);
		}
		$this->redirect('/admin/obstacles');
	}
	...
	function admin_import() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Obstacle']['save']) && $this->data['Obstacle']['save'] == '1') {
				$saveData = array();
				foreach($this->data['Obstacle']['images'] as $image) {
					if(rename(WWW_ROOT . 'img' . DS . 'admin' . DS . 'tmp' . DS . $image, WWW_ROOT . 'img' . DS . 'game' . DS . 'obstacles' . DS . $image)) {
						...
						$saveData[] = array('name' => '', 'image' => $image, 'group_id' => $this->data['Obstacle']['group_id'], 'sha1' => $sha1);
					}
				}
				...
				$sizes = getimagesize($this->data['Obstacle']['image']['tmp_name']);
				$src_im = imagecreatefrompng($this->data['Obstacle']['image']['tmp_name']);
