...
class ItemsController extends AppController {
	...
	function admin_edit($id = null) {
		...
		if(isset($this->data) && !empty($this->data)) {
			...
			if(isset($this->data['Item']['image']['name']) && !empty($this->data['Item']['image']['name'])) {
				// Oude verwijderen en nieuwe toevoegen...
				@unlink(WWW_ROOT . 'img' . DS . 'game' . DS . 'items' . DS . $oldItem['Item']['image']);
				eregi('((.)([A-Z]+))$', $this->data['Item']['image']['name'], $parts);
				$extentie = $parts[3];
				$file_name = uniqid(mt_rand(), true) .'.'. $extentie;
				move_uploaded_file($this->data['Item']['image']['tmp_name'], WWW_ROOT . 'img' . DS . 'game' . DS . 'items' . DS . $file_name);
				$this->data['Item']['image'] = $file_name;
				$this->data['Item']['icon'] = $file_name;
			}
			else {
				// Behoud de oude
			...
			if($this->Item->save($this->data)) {
				...
				foreach($this->data['ItemsStat'] as $index => $ItemsStat) {
					if($ItemsStat['check'] != 1) {
						unset($this->data['ItemsStat'][$index]);
					}
					else {
						$this->data['ItemsStat'][$index]['item_id'] = $id;
					}
				}
				$this->Item->ItemsStat->deleteAll(array('item_id' => $id));
				$this->Item->ItemsStat->saveAll($this->data['ItemsStat']);
