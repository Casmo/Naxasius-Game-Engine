 ...
 */
	...
	function admin_add($npc_id = null) {
		if(isset($this->data['Conversation']) && !empty($this->data['Conversation'])) {
		...
		if(isset($npc_id)) {
			...
			$this->data['Conversation']['npc_id'] = $this->data['Npc']['id'];
	...
	function admin_edit($id = null) {
		if(isset($this->data) && !empty($this->data)) {
			foreach($this->data['ActionsConversation'] as $index => $ActionsConversation) {
				if($ActionsConversation['check'] != '1') {
					unset($this->data['ActionsConversation'][$index]);
				}
				else {
					$this->data['ActionsConversation'][$index]['conversation_id'] = $this->data['Conversation']['id'];
				}
			}
			$this->Conversation->save($this->data);
			$this->Conversation->ActionsConversation->deleteAll(array('conversation_id' => $this->data['Conversation']['id']));
			$this->Conversation->ActionsConversation->saveAll($this->data['ActionsConversation']);
			$this->redirect('/admin/conversations');
		}
		$this->javascripts[] = '/js/tiny_mce/jquery.tinymce.js';
		...
		$this->set('conversations', $this->Conversation->find('list', array('conditions' => array('Conversation.npc_id' => $this->data['Conversation']['npc_id']), 'fields' => array('Conversation.id', 'Conversation.question'))));
