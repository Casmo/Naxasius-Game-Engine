	...
	function admin_save() {
		if(isset($this->data) && !empty($this->data)) {
			if(isset($this->data['Text']['id'])) {
				$text = $this->Text->find('first', array('conditions' => array('Text.id' => $this->data['Text']['id'])));
			}
			// resize
			...
			if(empty($this->data['Text']['image']['name'])) {
				...
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['Text']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['Text']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'website'. DS .'texts'. DS . $imageName);
				$Image->resize(WWW_ROOT . 'img'. DS .'website'. DS .'texts'. DS . $imageName, WWW_ROOT . 'img'. DS .'website'. DS .'texts'. DS . $imageName, 100, 100);
				$this->data['Text']['image'] = $imageName;
