...
class NpcsController extends AppController {
	...
	function admin_save($id = null) {
		...
		if(isset($this->data) && !empty($this->data)) {
			...
			$Image = new Image();
			if(empty($this->data['Npc']['image']['name'])) {
				unset($this->data['Npc']['image']);
			}
			else {
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['Npc']['image']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['Npc']['image']['tmp_name'], WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName);
				$Image->resize(WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName, WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName, 64, 64);
				$this->data['Npc']['image'] = $imageName;
				if(isset($npc['Npc']['image'])) {
					@unlink(WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $npc['Npc']['image']);
				}
			}
			if(empty($this->data['Npc']['icon']['name'])) {
				unset($this->data['Npc']['icon']);
			}
			else {
				$ext = preg_replace('/(.*)(\.(.*))$/', '\\3', $this->data['Npc']['icon']['name']);
				$imageName = md5(uniqid(mt_rand(), true)) .'.'. $ext;
				move_uploaded_file($this->data['Npc']['icon']['tmp_name'], WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName);

				$Image->resize(WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName, WWW_ROOT . 'img'. DS .'game'. DS .'npcs'. DS . $imageName, 32, 32);
				$this->data['Npc']['icon'] = $imageName;
