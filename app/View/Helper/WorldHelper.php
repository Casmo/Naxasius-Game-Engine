...
#						}
				...
				if(isset($options['playerPosition']) && $options['playerPosition'] == $areas[$index]['Area']['id']) {
					$html .= $this->Html->image('/img/game/avatars/'. $this->Session->read('Game.Avatar.image_map') .'/'. $this->Session->read('Game.Character.last_move') .'.png', array('alt' => __('You'), 'title' => __('You'), 'style' => 'z-index: 50;position: absolute; top: 3px; left: 8px;'));
				}
				$html .= '</div>';
				$index++;
			}
