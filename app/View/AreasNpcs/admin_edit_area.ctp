...
foreach($npcs as $npc) {
	...
	echo '<label for="InputNpcId'. $npc['Npc']['id'] .'">'. $this->Htmlimage('/img/game/npcs/'. $npc['Npc']['image']) .' '. $npc['Npc']['name'] .'</label> ';
	echo '<input type="input" value="'. $x .'" name="data[Npc]['. $index .'][x]" size="1"> '. __('X-pos') .' ';
	echo '<input type="input" value="'. $y .'" name="data[Npc]['. $index .'][y]" size="1"> '. __('Y-pos') .' ';
	echo '<input type="input" value="'. $z .'" name="data[Npc]['. $index .'][z]" size="1"> '. __('z-pos') .' ';
	...
	echo '<input type="file" name="data[Npc]['. $index .'][image]"> '. __('image') .'<br />';
	$index++;
}
