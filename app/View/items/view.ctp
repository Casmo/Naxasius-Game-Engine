...
?>
<div class="item_c">
<div class="item" id="item_<?php echo $item['Item']['id']; ?>"><?php echo $this->Html->image('/img/game/items/'. $item['Item']['icon']); ?></div>
<h1><?php echo $item['Item']['name']; ?></h1>
<p>
<?php echo $item['Item']['unique'] == '1' ? __('Unique') .'<br />' : ''; ?>
...
if(isset($item['Character']['name']) && !empty($item['Character']['name'])) {
	?>
	<div class="item_lootedby"><?php echo __('Discovered by:'); ?> <?php echo $item['Character']['name']; ?></div>
	<?php
}
?>
</div>
