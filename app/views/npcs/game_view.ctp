...
<div class="npc_c">
<div class="npc_text"><p><?php echo $ubb->output($conversations['last_log']['Conversation']['answer'], true); ?></p></div>
...
if(empty($conversations['available'])) {
	...
	foreach($conversations['available'] as $conversation) {
		...
		echo $ubb->output($conversation['Conversation']['question'], true) .'</div>';
	}
}
?>
</div>
