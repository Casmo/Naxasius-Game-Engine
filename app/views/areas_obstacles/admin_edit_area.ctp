...
$index = 0;
echo '<p><b>'. __('Current obstacles', true) .'</b></p>';
foreach($selectedObstacles as $index => $obstacle) {
	...
	echo '<label for="InputObstacleId'. $obstacle['id'] .'">'. $html->image('/img/game/obstacles/'. $obstacle['image']) .'</label> ';
	echo '<input type="input" value="'. $obstacle['x'] .'" name="data[Obstacle]['. $index .'][x]" size="1"> '. __('X-pos', true) .' ';
	echo '<input type="input" value="'. $obstacle['y'] .'" name="data[Obstacle]['. $index .'][y]" size="1"> '. __('Y-pos', true) .' ';
	echo '<input type="input" value="'. $obstacle['z'] .'" name="data[Obstacle]['. $index .'][z]" size="1"> '. __('Z-pos', true) .' ';
	...
	echo '<a href="#edit" onclick="popup(\''. $html->url('/admin/drops/edit/'. $obstacle['id']) .'\');">'. __('Edit Drops', true) .'</a>';
	echo '</p>';
}
